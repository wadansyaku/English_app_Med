  ...
  return (
    <div className="mx-auto mt-6 max-w-5xl space-y-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-600 transition-colors hover:bg-slate-50"
      >
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-3">
              {PLAN_PREVIEWS.map((plan) => (
                <div key={plan.plan} className="rounded-3xl border border-slate-200 bg-slate-50/70 px-5 py-5">
                  <div className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">{plan.audienceLabel}</div>
                  <div className="mt-2 text-lg font-black text-slate-950">{plan.label}</div>
                  <div className="mt-1 text-sm font-black text-medace-700">{plan.priceLabel}</div>
                  <p className="mt-3 text-sm leading-relaxed text-slate-600">{plan.pricingNote}</p>
                </div>
              ))}
            </div>
          </section>
